[//]: # (This template replaces README.md when someone creates a new repo with the fastpages template.)

![](https://github.com/noklam/mediumnok/workflows/CI/badge.svg) 
![](https://github.com/noklam/mediumnok/workflows/GH-Pages%20Status/badge.svg) 
[![](https://img.shields.io/static/v1?label=fastai&message=fastpages&color=57aeac&labelColor=black&style=flat&logo=data:image/png;base64,)](https://github.com/fastai/fastpages)

https://noklam.github.io/mediumnok/

# Not a Data Scientist
_powered by [fastpages](https://github.com/fastai/fastpages)_

fastpages allow me to blog directly in Notebook, so I don't have to worry how to convert into markdown anymore. I simple code and write. It also migrate my old markdown post with `docusaurus` seamlessly.

This is my blog, learning journal and my Dropbox of code.

I find that I have generated way too many repository and notebook, I hope this can become a central repository of my daily exploration, and hopefully I can search my own stuff on Google eventually (Which is faster thatn my Window Explorer). The blog is generated by Docusaurus ✔ with Markdown only. This blog is mean to be a logging journal of myself. I will keep the polished article on Medium [Medium](https://medium.com/@nokkk)). If you are interested to create a repository for yourself, you can follow my steps [Instruction](#instruction)


# Resource
I am generally interested in tools that increase productivity, please let me know if you have any recommendations. Here is a list of software/topics that I found useful.

## Uncertainty Estimation
[Uncertainty Quantification in Deep Learning](https://www.inovex.de/blog/uncertainty-quantification-deep-learning/)

## Visualization
[Visualization (University of Washington)](https://observablehq.com/collection/@uwdata/visualization-curriculum)


## Useful Python Tools
* pyinstructment: for profiling python process, which is useful for optimization
* torchsnooper -> pytorch profiling, another profiling tool which is for PyTorch, no more print x.shape anymore.
* knockknock notification: A single line of code that get you notifications when your 10 hours model training finally done. No more starring at the progress bar.

## Reviewing (any suggestions for code metric report/analysis library are welcome!)
* [coala](https://github.com/coala/coala) - coala provides a unified command-line interface for linting and fixing all your code, regardless of the programming languages you use.
* [radon](https://github.com/rubik/radon) - Radon is a Python tool that computes various metrics from the source code 

# Syntax Highlight
* lunr.js

